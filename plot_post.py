...
def plot_post(sample, alpha=0.05, show_mode=True, kde_plot=True, bins=50,
    ...
    post_summary = dict(mean=0, median=0, mode=0, alpha=0, hpd_low=0,
    ...
    if show_mode:
        ...
        plt.plot(0, label='mode =' + string.format(*post_summary['mode']),
                 alpha=0)
    else:
    ...
    for value in hpd:
        ...
        hpd_label = hpd_label + '{:g} {:g}\n'.format(round(value[0], roundto),
                                                     round(value[1], roundto))
    plt.plot(0, 0, linewidth=4, color='b',
             label='hpd {:g}%\n{}'.format((1 - alpha) * 100, hpd_label))
    # Display the ROPE.
    if ROPE is not None:
        pc_in_ROPE = round(
            np.sum((sample > ROPE[0]) & (sample < ROPE[1])) / len(
                sample) * 100, roundto)
        plt.plot(ROPE, [0, 0], linewidth=20, color='r', alpha=0.75)
        plt.plot(0, 0, linewidth=4, color='r',
    ...
    if comp_val is not None:
        pc_gt_comp_val = round(100 * np.sum(sample > comp_val) / len(sample),
                               roundto)
